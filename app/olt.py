...
def parse_index(oid):
    ...
    Format OID bisa berbeda tergantung vendor:
    - ZTE C300/C320: ...board.pon.onu_id (3 angka terakhir)
    - Tapi index bisa encoded dalam format lain
    """
    parts = oid.split('.')

    # Coba ambil 3 bagian terakhir
    if len(parts) >= 3:
        try:
            ...
            # Jika nilai terlalu besar, kemungkinan encoding berbeda
            # Format ZTE: board*16777216 + pon*65536 + onu_id
            if idx1 > 1000000:  # Encoded format
                board = idx1 // 16777216
                pon = (idx1 % 16777216) // 65536
                onu_id = idx3
...
def convert_hex_to_ascii(value):
    ...
    if isinstance(value, bytes):
        try:
            ...
            return ''.join(c for c in result if 32 <= ord(c) <= 126)
        except:
            return value.hex().upper()
...
def main():
    ...
    for oid, serial_raw in serial_data.items():
        ...
        # Ambil data lainnya
        name = convert_hex_to_ascii(name_data.get(
            oid, f"ONU-{board}/{pon}/{onu_id}"))
        desc = convert_hex_to_ascii(desc_data.get(oid, ""))
        ...
        print(f"{serial:16} | {status:10} | RX={rx_str} | TX={tx_str} | Dist={dist_str}m | {board}/{pon}/{onu_id}")
