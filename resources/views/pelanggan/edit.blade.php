<x-app.app-layout>
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            ...
            <div>
                <h1 class="text-3xl font-bold text-white">Edit Pelanggan</h1>
                <p class="text-gray-200 mt-1">Update data pelanggan</p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('pelanggan.update', $pelanggan) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $pelanggan->serial_number) }}" required
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="online" {{ old('status', $pelanggan->status) === 'online' ? 'selected' : '' }}>Online</option>
                        <option value="offline" {{ old('status', $pelanggan->status) === 'offline' ? 'selected' : '' }}>Offline</option>
                        <option value="los" {{ old('status', $pelanggan->status) === 'los' ? 'selected' : '' }}>LOS</option>
                        <option value="dyinggasp" {{ old('status', $pelanggan->status) === 'dyinggasp' ? 'selected' : '' }}>Dying Gasp</option>
                        <option value="dying" {{ old('status', $pelanggan->status) === 'dying' ? 'selected' : '' }}>Dying</option>
                        <option value="unknown" {{ old('status', $pelanggan->status) === 'unknown' ? 'selected' : '' }}>Unknown</option>
                    </select>
                    @error('status')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="olt_id" id="olt_id" value="{{ old('olt_id', $pelanggan->olt_id) }}"
                ...
                <div>
                    ...
                    <input type="number" name="board" id="board" value="{{ old('board', $pelanggan->board) }}"
                ...
                <div>
                    ...
                    <input type="number" name="pon" id="pon" value="{{ old('pon', $pelanggan->pon) }}"
                ...
                <div>
                    ...
                    <input type="number" name="onu_id" id="onu_id" value="{{ old('onu_id', $pelanggan->onu_id) }}"
                ...
                <div>
                    ...
                    <input type="text" name="onu_type" id="onu_type" value="{{ old('onu_type', $pelanggan->onu_type) }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="rx_power" id="rx_power" value="{{ old('rx_power', $pelanggan->rx_power) }}" placeholder="e.g. -18.5"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="tx_power" id="tx_power" value="{{ old('tx_power', $pelanggan->tx_power) }}" placeholder="e.g. 2.5"
                ...
                <div>
                    ...
                    <input type="number" name="gpon_optical_distance" id="gpon_optical_distance" value="{{ old('gpon_optical_distance', $pelanggan->gpon_optical_distance) }}"
                ...
                <div>
                    ...
                    <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $pelanggan->ip_address) }}"
                ...
                <div>
                    ...
                    <input type="text" name="health" id="health" value="{{ old('health', $pelanggan->health) }}"
                ...
                <!-- Uptime -->
                <div>
                    <label for="uptime" class="block text-sm font-semibold text-gray-700 mb-2">
                        Uptime
                    </label>
                    <input type="text" name="uptime" id="uptime" value="{{ old('uptime', $pelanggan->uptime) }}"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-transparent @error('uptime') border-red-500 @enderror">
                    @error('uptime')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="offline_reason" id="offline_reason" rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-transparent @error('offline_reason') border-red-500 @enderror">{{ old('offline_reason', $pelanggan->offline_reason) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $pelanggan->description) }}</textarea>
                    @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div class="flex items-start gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5 text-blue-600 mt-0.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
                    </svg>
                    <div class="text-sm text-blue-700">
                        <p class="font-semibold mb-1">Informasi:</p>
                        <ul class="list-disc list-inside space-y-1">
                            <li>Jika status diubah ke "Online", last_online akan diupdate otomatis</li>
                            <li>Jika status diubah ke selain "Online", last_offline akan diupdate otomatis</li>
                            <li>Timestamp akan diupdate secara otomatis</li>
                        </ul>
                    </div>
                </div>
            </div>
